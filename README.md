...
- **Setup count:** estimated from the dominant axis of each cylindrical face's surface normal, bucketed into X/Y/Z and counted, as a proxy for how many times a 3-axis machine must reorient the part.
...
- **Setup detection is a proxy, not a rigorous computation.** It buckets cylindrical faces by surface normal rather than true axis direction, which means it can miscount: it has no signal for conical features like a tapered tooth profile (undercounts), and it treats a purely cosmetic rounded edge the same as a real bore (overcounts). It happens to match my own read of how the gear and bracket would be fixtured.
